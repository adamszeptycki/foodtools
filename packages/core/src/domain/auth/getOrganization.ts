...
const getOrganization = async (userId: string) => {
	...
	const sortedMemberships = user.organizationMemberships
		.slice()
		.sort((a, b) => {
 ...
 */
const getOrganizationOptimized = async (
	userId: string,
	...
	if (sessionOrganizationId) {
		try {
			...
			console.warn(
				`Could not load organization ${sessionOrganizationId} from session, falling back to full resolution`,
			);
		}
	}
