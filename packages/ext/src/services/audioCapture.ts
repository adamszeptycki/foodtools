...
export class AudioCaptureService {
	...
	static async saveRecording(
		recording: Omit<AudioRecording, "id" | "timestamp">,
		...
		const recordings = await AudioCaptureService.getAllRecordings();
		recordings.push(newRecording);

		await chrome.storage.local.set({
			[AudioCaptureService.STORAGE_KEY]: recordings,
		});
	...
	static async getAllRecordings(): Promise<AudioRecording[]> {
		const result = await chrome.storage.local.get([
			AudioCaptureService.STORAGE_KEY,
		]);
		return result[AudioCaptureService.STORAGE_KEY] || [];
	}
	...
	static async deleteRecording(id: string): Promise<void> {
		...
		await chrome.storage.local.set({
			[AudioCaptureService.STORAGE_KEY]: filtered,
		});
	}
	...
	static downloadRecording(recording: AudioRecording, filename?: string): void {
		const url = AudioCaptureService.createBlobUrl(recording.data);
